...
const baseFixture = (type, current = {}) => {
  ...
  const nationality =
    current.nationality ||
    (type === "afghan" ? "Afghanistan" : type === "voa" ? "Japan" : "Canada");
  const countryOfBirth =
    current.country_of_birth ||
    (type === "afghan" ? "Afghanistan" : nationality);
  ...
  const common = {
    ...
    present_address:
      current.present_address || `100 Example Street, ${nationality}`,
    ...
    father_details:
      current.father_details || `JAMES MORGAN; ${nationality}; City`,
    mother_details:
      current.mother_details || `SARAH MORGAN; ${nationality}; City`,
    ...
    employer_address:
      current.employer_address || `200 Sample Road, ${nationality}`,
    ...
    india_reference:
      current.india_reference ||
      "Central Hotel, Example Road, Delhi; +910000000000",
    home_reference:
      current.home_reference ||
      `Home Contact, 100 Example Street; +10000000002`,
    ...
    confirm_email:
      current.confirm_email || current.email || "alex.morgan@example.invalid",
    expected_arrival_date: current.expected_arrival_date || futureDate(45),
    instructions_ready:
      current.instructions_ready !== undefined
        ? current.instructions_ready
        : true,
  ...
  if (type === "voa")
    return {
      ...
      no_india_residence_occupation:
        current.no_india_residence_occupation !== undefined
          ? current.no_india_residence_occupation
          : true,
      onward_ticket_confirmed:
        current.onward_ticket_confirmed !== undefined
          ? current.onward_ticket_confirmed
          : true,
      sufficient_funds_confirmed:
        current.sufficient_funds_confirmed !== undefined
          ? current.sufficient_funds_confirmed
          : true,
      uae_previous_indian_visa:
        current.uae_previous_indian_visa ||
        (nationality === "United Arab Emirates" ? "yes" : "not_applicable"),
      pakistan_origin: current.pakistan_origin || "no",
      ...
      passport_expiry_date: current.passport_expiry_date || "2034-01-14",
      permanent_address:
        current.permanent_address || `100 Example Street, ${nationality}`,
      ...
      voa_truthful:
        current.voa_truthful !== undefined ? current.voa_truthful : true,
      voa_airport_process:
        current.voa_airport_process !== undefined
          ? current.voa_airport_process
          : true,
      voa_nonextendable:
        current.voa_nonextendable !== undefined
          ? current.voa_nonextendable
          : true,
    };
  if (type === "afghan") {
    const category = current.visa_category || "medical";
    const purpose =
      current.afghan_purpose || afghanPurposes[category]?.[0] || "";
    return {
      ...common,
      ...
      address_in_india:
        current.address_in_india || "Hospital Guest House, Delhi",
      principal_applicant_id:
        category === "medical-attendant" ||
        ["business-dependant", "student-dependant"].includes(purpose)
          ? current.principal_applicant_id || "VS2026A00001"
          : current.principal_applicant_id,
  ...
  if (type === "oci") {
    ...
    const isDescendant = ["descendant-child", "descendant-grandchild"].includes(
      ociCat,
    );
    const country =
      current.country_of_application || current.nationality || "Canada";
    ...
    return {
      ...
      application_center:
        current.application_center || "High Commission of India, Ottawa",
      email: current.email || "alex.morgan@example.invalid",
      ...
      gender:
        current.gender && ["male", "female", "other"].includes(current.gender)
          ? current.gender
          : "male",
      date_of_birth: current.date_of_birth || "1992-05-14",
      place_of_birth:
        current.place_of_birth || (isDescendant ? "Toronto" : "Mumbai"),
      country_of_birth:
        current.country_of_birth || (isDescendant ? country : "India"),
      marital_status:
        current.marital_status || (isSpouse ? "married" : "single"),
      ...
      foreign_citizenship_date:
        current.foreign_citizenship_date || "2022-01-15",
      foreign_citizenship_mode:
        current.foreign_citizenship_mode ||
        (isDescendant ? "birth" : "naturalization"),
      naturalization_cert_no:
        current.naturalization_cert_no ||
        (isDescendant ? "NA" : "CAN-NAT-2022-9988"),

      father_name: current.father_name || "RAJESH MORGAN",
      father_nationality:
        current.father_nationality || (isDescendant ? country : "India"),
      father_former_indian: current.father_former_indian || "yes",
      mother_name: current.mother_name || "PRIYA MORGAN",
      mother_nationality:
        current.mother_nationality || (isDescendant ? country : "India"),
      mother_former_indian: current.mother_former_indian || "yes",
      spouse_name: current.spouse_name || (isSpouse ? "PRIYA SHARMA" : ""),
      spouse_nationality:
        current.spouse_nationality || (isSpouse ? "India" : ""),
      spouse_oci_or_indian:
        current.spouse_oci_or_indian || (isSpouse ? "yes" : "na"),
      marriage_date: current.marriage_date || (isSpouse ? "2020-06-18" : ""),

      basis_of_origin:
        current.basis_of_origin ||
        (ociCat === "former-indian"
          ? "self-former-indian"
          : isSpouse
            ? "foreign-spouse-basis"
            : ociCat === "descendant-grandchild"
              ? "grandparent-origin"
              : "parent-origin"),
      previous_indian_passport:
        current.previous_indian_passport ||
        (ociCat === "former-indian" ? "Z1234567" : "NA"),
      surrender_cert_number:
        current.surrender_cert_number ||
        (ociCat === "former-indian" ? "SC/OTT/2022/9941" : "NA"),
      ancestor_name_details:
        current.ancestor_name_details ||
        (isDescendant
          ? "Father Mr. Rajesh Morgan born in New Delhi in 1965, Indian Passport No: A8765432"
          : ""),
      permanent_address:
        current.permanent_address ||
        `100 Example Street, Ottawa, ON, ${country}`,
      ...
      review_accuracy:
        current.review_accuracy !== undefined ? current.review_accuracy : true,
    };
  }
  return {
    ...common,
    ...
    visa_validity: current.visa_validity || "30-days",
    student_course_type:
      current.visa_category === "student"
        ? current.student_course_type || "general-course"
        : current.student_course_type,
...
export function demoFixture(type, current = {}) {
  const normalized = { ...current };
  for (const step of getSteps(type, current))
    for (const item of step.fields || []) {
      if (
        item.type === "select" &&
        item.options?.includes("yes") &&
        typeof normalized[item.name] === "boolean"
      )
        normalized[item.name] = normalized[item.name] ? "yes" : "no";
    }
  const data = { ...normalized, ...baseFixture(type, normalized) };
  // Populate fields revealed by an existing choice, without changing that choice.
  for (let pass = 0; pass < 2; pass++)
    for (const step of getSteps(type, data))
      for (const item of step.fields || []) {
        if (
          !isVisible(item, data) ||
          ...
          continue;
        if (item.name === "review_accuracy" || item.type === "checkbox")
          continue;
        data[item.name] =
          item.type === "select"
            ...
            : item.type === "date"
              ...
              : item.type === "number"
                ...
                : item.name.includes("phone")
                  ? "+10000000000"
                  : "Example information for testing";
      }
  // A human must review the final declaration even after Autofill.
  data.review_accuracy = current.review_accuracy === true;
  return data;
}
